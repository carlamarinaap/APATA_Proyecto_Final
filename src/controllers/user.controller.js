...
export async function getUsers(req, res) {
  try {
    ...
    req.logger.INFO(`Se devolvieron todos los usuarios`);
    res.status(200).send(allUsers);
  } catch (error) {
    req.logger.ERROR(error.message);
    res.status(500).send(`Hubo un error obteniendo los usuarios: ${error.message}`);
  }
}
...
export async function deleteUser(req, res) {
  try {
    ...
    await userService.delete({ _id: uid });
    req.logger.INFO(`Se eliminó el usuario ${user.email}`);
    res.send(`Se eliminó el usuario ${user.email}`);
  } catch (error) {
    res.status(500).send(`Hubo un error elimiando el usuario: ${error.message}`);
  }
}
  ...
  try {
    ...
    if (userId === 1) {
      ...
      allUsers.forEach((user) => {
        const diffConnection = new Date() - new Date(user.last_connection);
        const diffInDays = diffConnection / (1000 * 60 * 60 * 24);
        diffInDays > 2 && userSchema.deleteOne(user._id);
      });
      const users = await userService.get();
      req.logger.INFO(`Se eliminaron los usuarios con más de dos días sin conectarse`);
  ...
  try {
    ...
    if (user && user.role === "user") {
      ...
      if (filtro.length === 3) {
        ...
        req.logger.ERROR("El usuario no ha terminado de procesar su documentacion");
...
export async function userAdminView(req, res) {
  try {
    if (req.signedCookies.jwt) {
      const userId = jwt.verify(req.signedCookies.jwt, config.privateKey).id;
      if (userId === 1) {
      ...
      let msg = "Debe loguearse para acceder";
      res.render("login", { msg });
    }
  } catch (error) {
    res.status(500).send(error.message);
